...
namespace PathCreation
{
	...
	public class BezierPath
	{
		...
		///<param name="isClosed"> Should the end point connect back to the start point? </param>
		///<param name="space"> Determines if the path is in 3d space, or clamped to the xy/xz plane </param>
		public BezierPath(IEnumerable<Vector4> points, bool isClosed = false, PathSpace space = PathSpace.xyz)
		{
			...
			else
			{
				...
				this.points = new List<Vector4> { pointsArray[0], new Vector4(0,0,0, pointsArray[0].w), new Vector4(0, 0, 0, pointsArray[1].w), pointsArray[1] };
		...
		public BezierPath(IEnumerable<Transform> transforms, bool isClosed = false, PathSpace space = PathSpace.xy) :
			 this(transforms.Select(t => new Vector4(t.position.x, t.position.y, t.position.z)), isClosed, space)
		{ }
		...
		public BezierPath(IEnumerable<Vector2> points, PathSpace space = PathSpace.xyz, bool isClosed = false) :
			 this(points.Select(p => new Vector4(p.x, p.y)), isClosed, space)
		{ }
		...
		/// Get world space position of point
		public void SetPoint(int i, Vector4 localPosition, bool suppressPathModifiedEvent = false)
		{
		...
		public void AddSegmentToEnd(Vector4 anchorPos)
		{
			if (isClosed)
			{
				return;
			...
			Vector4 secondControlForOldLastAnchorOffset = (points[lastAnchorIndex] - points[lastAnchorIndex - 1]);
		...
		public void AddSegmentToStart(Vector4 anchorPos)
		{
			if (isClosed)
			{
				return;
			...
			// Set position for new control to be mirror of its counterpart
			Vector4 secondControlForOldFirstAnchorOffset = (points[0] - points[1]);
			if (controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic)
			{
				// Set position for new control to be aligned with its counterpart, but with a length of half the distance from prev to new anchor
			...
			Vector4 secondControlForOldFirstAnchor = points[0] + secondControlForOldFirstAnchorOffset;
			Vector4 controlForNewAnchor = (anchorPos + secondControlForOldFirstAnchor) * .5f;
		...
		public void SplitSegment(Vector4 anchorPos, int segmentIndex, float splitTime)
		{
			splitTime = Mathf.Clamp01(splitTime);

			if (controlMode == ControlMode.Automatic)
			{
				points.InsertRange(segmentIndex * 3 + 2, new Vector4[] { Vector3.zero, anchorPos, Vector3.zero });
			...
			else
			{
				...
				Vector4[][] splitSegment = CubicBezierUtility.SplitCurve(GetPointsInSegment(segmentIndex), splitTime);
				points.InsertRange(segmentIndex * 3 + 2, new Vector4[] { splitSegment[0][2], splitSegment[1][0], splitSegment[1][1] });
		...
		public Vector4[] GetPointsInSegment(int segmentIndex)
		{
			segmentIndex = Mathf.Clamp(segmentIndex, 0, NumSegments - 1);
			return new Vector4[] { this[segmentIndex * 3], this[segmentIndex * 3 + 1], this[segmentIndex * 3 + 2], this[LoopIndex(segmentIndex * 3 + 3)] };
		}
				...
				else
				{
					...
					else if (controlMode != ControlMode.Free)
					{
						...
						if (attachedControlIndex >= 0 && attachedControlIndex < points.Count || isClosed)
						{
							...
							Vector4 dir = (points[LoopIndex(anchorIndex)] - pointPos).normalized;
		...
		void AutoSetAnchorControlPoints(int anchorIndex)
		{
			...
			Vector4 anchorPos = points[anchorIndex];
			Vector4 dir = Vector4.zero;
			float[] neighbourDistances = new float[2];

			if (anchorIndex - 3 >= 0 || isClosed)
			{
				Vector4 offset = points[LoopIndex(anchorIndex - 3)] - anchorPos;
		...
		void AutoSetStartAndEndControls()
		{
			if (isClosed)
			{
				...
				if (NumAnchorPoints == 2)
				{
					Vector4 dirAnchorAToB = (points[3] - points[0]).normalized;
					float dstBetweenAnchors = (points[0] - points[3]).magnitude;
					Vector4 perp = Vector3.Cross(dirAnchorAToB, (space == PathSpace.xy) ? Vector3.forward : Vector3.up);
