	...
	public class PathEditor : Editor
	{
					...
					if (data.showTransformTool & (handleIndexToDisplayAsTransform == -1))
					{
						if (GUILayout.Button("Centre Transform"))
						{
							...
							if (worldCentre != creator.transform.position)
							{
								...
								{
									Vector3 lcentre = MathUtility.InverseTransformVector(worldCentreToTransform, creator.transform, bezierPath.Space);
									for (int i = 0; i < bezierPath.NumPoints; i++)
									{
										bezierPath.SetPoint(i, 
											bezierPath.GetPoint(i) + new Vector4(lcentre.x, lcentre.y, lcentre.z), true);
									}
								}
		...
		void DrawVertexPathSceneEditor()
		{
			...
			if (data.showBezierPathInVertexMode)
			{
				...
				{
					Vector4[] points = bezierPath.GetPointsInSegment(i);
			...
			if (data.showNormalsInVertexMode)
			{
				...
				for (int i = 0; i < creator.path.NumPoints; i++)
				{
					...
					normalLines[i * 2 + 1] = (Vector3)creator.path.GetPoint(i) + creator.path.localNormals[i] * globalDisplaySettings.normalsLength;
				}
			...
			if (mouseOverHandleIndex == -1)
			{
				if (e.type == EventType.MouseDown && e.button == 0 && e.shift)
				{
					...
					else if (!bezierPath.IsClosed)
					{
						...
						float dstCamToEndpoint = (Camera.current.transform.position - (Vector3)bezierPath[bezierPath.NumPoints - 1]).magnitude;
			...
			if (Event.current.type == EventType.Repaint)
			{
				...
				if (data.showNormals)
				{
					...
					for (int i = 0; i < normalsVertexPath.NumPoints; i++)
					{
						...
						normalLines[i * 2 + 1] = (Vector3)normalsVertexPath.GetPoint(i) + normalsVertexPath.GetNormal(i) * globalDisplaySettings.normalsLength;
					}
					Handles.DrawLines(normalLines);
				}
			}
		...
		void DrawHandle(int i)
		{
			...
			if (doTransformHandle)
			{
				...
				if (data.showNormals && Tools.current == Tool.Rotate && isAnchorPoint && bezierPath.Space == PathSpace.xyz)
				{
					...
					Vector3 dir = ((Vector3)bezierPath[attachedControlIndex] - handlePosition).normalized;
			...
			if ((Vector3)bezierPath[i] != localHandlePosition)
			{
				Undo.RecordObject(creator, "Move point");
